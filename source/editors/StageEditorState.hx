...
class StageEditorState extends MusicBeatState
{
	...
	override function create()
	{
		...
		FlxG.cameras.add(camPeople, false);
		FlxG.cameras.add(camHUD, false);
		FlxG.cameras.add(camMenu, false);
		FlxG.cameras.add(camTips, false);
	...
	function reloadStages() {
		...
		for (layer in stageFile.layerArray) {
            ...
            #else
            if (Assets.exists('' + Paths.image(assetName))) { //have to add the '' or else it gives an error???? - crushernotdrip
            #end
			createdLayer = new FlxSprite();
	...
	function searchForLayer()
	{
		...
		if (assetName != null && assetName.length > 0)
		{
            ...
            #else
            if (Assets.exists('' + Paths.image(directoryLayer))) //have to add the ''  + or else it gives an error???? - crushernotdrip
            #end
			{
